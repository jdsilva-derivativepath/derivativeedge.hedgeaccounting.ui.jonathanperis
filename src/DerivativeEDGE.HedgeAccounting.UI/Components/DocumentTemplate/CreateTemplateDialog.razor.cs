using static DerivativeEDGE.HedgeAccounting.UI.Pages.DocumentTemplate.CreateTemplate;

namespace DerivativeEDGE.HedgeAccounting.UI.Components.DocumentTemplate;

public partial class CreateTemplateDialog
{
    [Parameter]
    public CreateTemplateModel CreateTemplateForm { get; set; } = new();

    [Parameter]
    public long? SelectedClientId { get; set; }

    [Parameter]
    public EventCallback<CreateTemplateModel> OnSave { get; set; }

    [Parameter]
    public bool Visibility { get; set; } = false;

    public DocumentTemplateActionResult DocumentTemplateActionResult { get; set; } = new DocumentTemplateActionResult(DocumentTemplateActions.Create, true);

    [Inject]
    private IMediator MediatorService { get; set; } = default!;

    private string HeaderText => CreateTemplateForm.Id == Guid.Empty ? "New Template" : "Edit Template Details";

    private CustomValidation _customValidation;

    private bool IsMessageVisible { get; set; } = false;
    private bool IsEditMode { get; set; } = false;

    private static ErrorMessage CharacterCountValidation => new()
    {
        Title = "Some fixes are needed",
        Message = "Please update the highlighted fields to proceed."
    };

    private CreateTemplateModel OldModel { get; set; }
    public void DlgRenameOpen(bool editMode = false)
    {
        Visibility = true;
        IsEditMode = editMode;
        IsMessageVisible = false;
        OldModel = CreateTemplateForm.GetClone();
    }

    public async Task DlgRenameClose()
    {
        Visibility = false;
        CreateTemplateForm = OldModel;
        await OnSave.InvokeAsync(CreateTemplateForm);
    }

    private async Task SubmitInitialCreateForm(EditContext editContext)
    {
        var isValid = editContext.Validate();
        var updateObj =  (CreateTemplateModel) editContext.Model;
        IsMessageVisible = !(isValid && await CustomValidation());

        if (IsMessageVisible)
        {
            return;
        }

        Visibility = false;
        await OnSave.InvokeAsync(updateObj);
        StateHasChanged();
    }

    private async Task<bool> CustomValidation()
    {
        try
        {
            _customValidation?.ClearErrors();

            var errors = new Dictionary<string, List<string>>();

            var documentTemplates = await MediatorService.Send(new ListDocumentTemplates.Query(SelectedClientId.GetValueOrDefault(0)));
            if (documentTemplates.DocumentTemplates
                .Any(c => string.Equals(c.Name, CreateTemplateForm.Name?.TrimEnd(), StringComparison.OrdinalIgnoreCase)) &&
                DocumentTemplateActionResult.Action == DocumentTemplateActions.Create)
            {
                errors.Add(nameof(Models.DocumentTemplate.Name),
                        ["Hedge Document Template name must be unique."]);
            }

            if (errors.Count != 0)
            {
                _customValidation?.DisplayErrors(errors);
                return false;
            }
            return true;
        }
        catch (Exception)
        {
            Visibility = false;
            
            return false;
        }
    }
}
