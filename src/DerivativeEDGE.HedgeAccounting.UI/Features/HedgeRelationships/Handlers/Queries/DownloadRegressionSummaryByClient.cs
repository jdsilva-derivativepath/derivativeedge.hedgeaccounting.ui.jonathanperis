...
public sealed class DownloadRegressionSummaryByClient
{
    ...
    public sealed class Handler(IHedgeAccountingApiClient hedgeAccountingApiClient, TokenProvider tokenProvider, ILogger<DownloadRegressionSummaryByClient.Handler> logger) : IRequestHandler<Query, Response>
    {
        public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    fileResponse = await hedgeAccountingApiClient.SummaryAsync(request.id ?? 0, valueDate, cancellationToken);
                }
                ...
                {
                    logger.LogWarning("{Client}.{Method} returned null FileResponse (id: {Id}, valueDate: {ValueDate})", clientTypeName, methodName, request.id, request.ValueDate);
